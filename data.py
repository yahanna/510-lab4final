...
def fetch_and_store_weather_data(api_key, city_name, country):
    ...
    if response.status_code == 200:
        ...
        # 确保先有对应的location数据
        # cursor.execute("INSERT INTO locations (city_name, country) VALUES (?, ?)", (city_name, country))
        # conn.commit()
        ...
        insert_query = """
            INSERT INTO weather_data (city_name, country_name, temperature, humidity, weather_description)
            VALUES (?, ?, ?, ?, ?);
        """
        cursor.execute(insert_query, (city_name, country, temperature, humidity, weather_description))
